...
Real-world automation scripts I built for a county government software company. These scripts replaced hours of manual work with automated verification and reporting.

## Scripts

### `eom-check.js` — End-of-Month Balance Verification

Uses [Playwright](https://playwright.dev/) to automate end-of-month balance checking across county accounting web applications. Instead of a person logging into each county's system, clicking through screens, and copying results — this script does it all automatically.

**What it does:**
1. Launches a headless browser
2. Navigates to the county's posting period page
...
export BASE_URL="https://{county}.yourapp.com"

node eom-check.js canadian
```
...
**Result:** What took a support person several hours every month-end now takes 5 minutes across all counties. [Read the full case study](https://birdherd.media/case/eom-automation)
...
2. For each county, queries multiple collections (tax payments, misc transactions, mortgage transactions)
...
# Single county
node apportionment-check.js --county canadian

# Multiple counties
node apportionment-check.js --counties "canadian,logan,carter"

# All counties
node apportionment-check.js --counties all

# Previous month with email report
node apportionment-check.js --counties all --month -1 --email

# JSON output for automation
node apportionment-check.js --county canadian --json
```

**Output:**
```
[1/1] Processing canadian...
  [1/5] Getting county config...
...
**Result:** What took a team 40+ hours per month (manually comparing two screens, fund by fund, for every county) now takes 2 minutes. [Read the full case study](https://birdherd.media/case/slack-bot)

---

## How These Fit Together

In production, these scripts are triggered by a Slack bot. A support person types `@Sentinel eom` or `@Sentinel apportionment canadian` in Slack, and results are posted back to the channel automatically. The scripts run on a server via SSH, orchestrated by [n8n](https://n8n.io/) workflow automation.
...
| `BASE_URL` | URL pattern with `{county}` placeholder | eom-check |
